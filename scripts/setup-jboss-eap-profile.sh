...
if [ -z "$JBOSS_BASE_DIR" ]
then
  $JBOSS_INSTALLATION_DIR/bin/standalone.sh -b 127.0.0.1 -bmanagement 127.0.0.1 -c $TARGET_PROFILE --admin-only 2>&1 > $JBOSS_CONSOLE_LOG &
else 
  $JBOSS_INSTALLATION_DIR/bin/standalone.sh -b 127.0.0.1 -bmanagement 127.0.0.1 -c $TARGET_PROFILE -Djboss.server.base.dir=$JBOSS_BASE_DIR --admin-only 2>&1 > $JBOSS_CONSOLE_LOG &
fi
...
# Now that the system is running, we can run our CLI scripts. They will be executed in the order defined by 'sort -V'
	...
	do 
		...
		$JBOSS_INSTALLATION_DIR/bin/jboss-cli.sh -c --timeout=10000 --file=$f
	done
fi
